...
function Actions.initialize(args, agent_state)
  ...
  log:debug('[Chain of Thought Agent] Initializing session: %s', args.problem)
  ...
  return {
    ...
    data = fmt(
      [[Problem: %s
CoT initialized - Session ID: %s

NEXT: Begin reasoning immediately by calling add_step with:
- action: "add_step"  
- step_id: "step1"
- content: "Your analysis of the problem"
- step_type: "analysis"

Continue with more add_step calls until problem is solved.

Actions: add_step, view_chain, reflect]],
      args.problem,
...
function Actions.view_chain(args, agent_state)
  ...
  log:debug('[Chain of Thought Agent] Viewing chain structure')
...
return {
  ...
  schema = {
    ...
    ['function'] = {
      name = 'chain_of_thoughts_agent',
      description = 'Sequential coding problem solver: ideal for debugging, implementing features, refactoring, and step-by-step development.',
      parameters = {
        type = 'object',
        properties = {
          action = {
