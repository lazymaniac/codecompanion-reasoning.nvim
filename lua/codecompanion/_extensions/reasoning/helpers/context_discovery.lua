...
---@param problem_type string Type of problem (e.g., 'authentication_bug', 'performance_issue')
---@param reasoning_steps table[] Array of reasoning steps that worked
---@param outcome string Outcome description
function ContextDiscovery.store_reasoning_pattern(problem_type, reasoning_steps, outcome)
  local memory = ContextDiscovery.load_memory()

  if not memory.reasoning_patterns[problem_type] then
    memory.reasoning_patterns[problem_type] = {}
  end
  ...
  table.insert(memory.reasoning_patterns[problem_type], pattern)

  -- Keep only last 5 patterns per problem type to avoid bloat
  local patterns = memory.reasoning_patterns[problem_type]
  if #patterns > 5 then
    table.remove(patterns, 1)
  end

  ContextDiscovery.save_memory(memory)
  log:debug('[Context Discovery] Stored reasoning pattern for: %s', problem_type)
end

---Get reasoning patterns for problem type
---@param problem_type string Type of problem
---@return table[] Array of reasoning patterns
function ContextDiscovery.get_reasoning_patterns(problem_type)
  local memory = ContextDiscovery.load_memory()
  return memory.reasoning_patterns[problem_type] or {}
end
...
---@param problem_description string Problem description
---@param solution_approach string How it was solved
---@param files_involved table[] Files that were part of the solution
function ContextDiscovery.store_problem_solution(problem_description, solution_approach, files_involved)
  local memory = ContextDiscovery.load_memory()
  ...
  table.insert(memory.problem_solutions, solution)

  -- Keep only last 20 solutions to avoid bloat
  if #memory.problem_solutions > 20 then
    table.remove(memory.problem_solutions, 1)
  end

  ContextDiscovery.save_memory(memory)
  log:debug('[Context Discovery] Stored problem-solution mapping')
end

---Search for similar problems and solutions
---@param problem_keywords table Keywords to search for
---@return table[] Array of matching solutions
function ContextDiscovery.search_similar_problems(problem_keywords)
  local memory = ContextDiscovery.load_memory()
  local matches = {}

  for _, solution in ipairs(memory.problem_solutions) do
    local problem_text = solution.problem:lower()
    local solution_text = solution.solution:lower()

    for _, keyword in ipairs(problem_keywords) do
      local kw = keyword:lower()
      if problem_text:find(kw, 1, true) or solution_text:find(kw, 1, true) then
        table.insert(matches, solution)
        break
      end
    end
  end
...
---@param tool_name string Tool that was used
---@param context_type string What type of task (e.g., 'file_editing', 'debugging')
---@param success boolean Whether the tool usage was successful
function ContextDiscovery.store_tool_usage(tool_name, context_type, success)
  local memory = ContextDiscovery.load_memory()

  local key = fmt('%s_%s', tool_name, context_type)
  if not memory.tool_usage_patterns[key] then
    memory.tool_usage_patterns[key] = {
      tool = tool_name,
  ...
  local pattern = memory.tool_usage_patterns[key]
  pattern.last_used = os.time()
...
---Get recommended tools for context type
---@param context_type string Type of task
---@return table[] Array of recommended tools sorted by success rate
function ContextDiscovery.get_recommended_tools(context_type)
  local memory = ContextDiscovery.load_memory()
  local recommendations = {}

  for key, pattern in pairs(memory.tool_usage_patterns) do
    if pattern.context == context_type then
      local total_uses = pattern.success_count + pattern.failure_count
      local success_rate = total_uses > 0 and (pattern.success_count / total_uses) or 0
  ...
  table.sort(recommendations, function(a, b)
    if math.abs(a.success_rate - b.success_rate) < 0.01 then
      return a.total_uses > b.total_uses
    end
